...
console.log('== 体験版（配る版）は、どう操作しても本物の記録に送らない ==');
{
  ...
  /* ★2026-08-19 変更：体験版では資料そのものを開かない。
     本部ドライブの資料は本部の方しか閲覧権限が無く（114件すべて未ログインで401）、
     タップすると Googleの「アクセス権が必要です」が出て、使えないアプリに見えるため。
     一覧（どこに何があるか）は今までどおり見えるようにしておく。 */
  ok(/本番のご利用開始後にご覧いただけます/.test(man), 'いつ見られるようになるかが画面に出る');
  ok(!/data-openurl="https/.test(man), '体験版から本部の資料を開かない');
  ok(!/アクセス権/.test(man), 'Googleの「アクセス権が必要です」へ飛ばさない');
