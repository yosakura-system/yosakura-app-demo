...
/* ★2026-08-26 削除＝「掃除1〜3」（2026-08-07の行番号決め打ちの後始末）はここから除去した。
   旧シートの行番号を前提にしていたため、新しいバックエンド（yosakura.fc）で実行すると
   無関係な行を消してしまう。行の掃除が必要になったら surveyRowsReport → runDeleteSurveyRows を使う。 */
